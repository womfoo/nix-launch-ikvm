{ stdenv, pkgs, curl, icedtea8_web, openjdk8, patchelf, xmlstarlet }:

assert stdenv.system == "x86_64-linux";

let

  run = pkgs.writeScript "launch-ikvm" ''
    #!/usr/bin/env bash
    ...
    ${xmlstarlet}/bin/xml ed -d "$XPATH_JAR_RESOURCES" "$1" > new-launch.jnlp
